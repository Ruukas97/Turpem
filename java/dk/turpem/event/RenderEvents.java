...
public class RenderEvents {	
	...
	@SubscribeEvent
	public void setPlayerNameTag(RenderLivingEvent.Specials.Pre event){
        //GL11.glAlphaFunc(GL11.GL_GREATER, 0.1F);

        //if (event.renderer.func_110813_b(event.entity))
        //{
            //double d3 = event.entity.getDistanceSqToEntity(event.renderer.renderManager.livingPlayer);
            //float f2 = event.entity.isSneaking() ? event.renderer.NAME_TAG_RANGE_SNEAK : event.renderer.NAME_TAG_RANGE;
		...
		String s;
				if(event.entity instanceof EntityPlayer) {
	            	...
	            	if(MathHelper.calculateDistanceEntities3D(client, player) <= event.renderer.NAME_TAG_RANGE_SNEAK){
			    	...
			    	/*Minecraft.getMinecraft().renderEngine.bindTexture(coder);			    	
			        GL11.glPushMatrix();
			        //GL11.glTranslated(player.posX, player.posY + 2, player.posZ);//+1 so that our "drawing" appears 1 block over our block (to get a better view)
			        GL11.glNormal3f(0.0F, 1.0F, 0.0F);
			        GL11.glRotatef(-playerViewY, 0.0F, 1.0F, 0.0F);
			        GL11.glRotatef(playerViewX, 1.0F, 0.0F, 0.0F);
			        
					Tessellator tesselator2 = Tessellator.instance;
			        tesselator2.startDrawingQuads();			        
			        tesselator2.addVertex(-0.5, 0.5, 0);
			        tesselator2.addVertex(0.5, 0.5, 0);
			        tesselator2.addVertex(-0.5, -0.5, 0);
			        tesselator2.addVertex(0.5, -0.5, 0);
					...
					/*tessellator.startDrawingQuads();
					tessellator.addVertex(0, 1, 0);//origin
					tessellator.addVertex(0, 1.5, 0);//1 unit up
					tessellator.addVertex(0.5, 1.5, 0);//1 unit up and to the right
					tessellator.addVertex(0.5, 1, 0);//1 unit to the right
					tessellator.draw();*/
				...
				//else
                //{
                    //event.renderer.func_96449_a(event.entity, event.x, event.y, event.z, s, f1, 5);
                //}
            //}
        //}
	}
